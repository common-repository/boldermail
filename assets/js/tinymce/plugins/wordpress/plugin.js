/**
 * Boldermail custom WordPress TinyMCE plugin.
 *
 * Replace the default WordPress TinyMCE plugin with a custom one.
 * Our custom plugin is identical to the WordPress one, except it does
 * not replace `<script>` and `<style>` tags with an image, AND
 * it prevents adding `<p> tags to comments (these comments are used by our
 * templates to specify Microsoft Office-specific code), AND it prevents
 * deletion of `<br>` tags. We could also prevent the second and third issues
 * using `wpautop => false` when setting up the editor,
 * but that also leads to unpredictable behavior with TinyMCE adding `<p>`
 * tags around the template.
 *
...
tinymce.PluginManager.add( 'bm_wordpress', function( editor ) {
